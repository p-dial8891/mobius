...
*/
	...
	while let Some(cmd) = rx.recv().await {
		...
		match cmd {
		    119 => { let uname = client.username(context::current(), "MasterPassword".to_string(), 
			         "Polaris".to_string()).await?; 
					 println!("{uname}"); }
			121 => { let pwd = client.password(context::current(), "MasterPassword".to_string(), 
			         "Polaris".to_string()).await?;
			         println!("{pwd}"); }
		    _   => { }
	    }
