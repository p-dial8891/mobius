...
use clap::Parser;
use service::{init_tracing, MobiusClient};
...
async fn main() -> anyhow::Result<()> {
    ...
    let mut transport = tarpc::serde_transport::tcp::connect(flags.server_addr
    ...
    while let Some(cmd) = rx.recv().await {
        ...
        match cmd {
            119 => { let uname = client.username(context::current(), flags.secret.clone(), 
                     flags.id.clone()).await?; 
                     println!("{uname}"); }
            121 => { let pwd = client.password(context::current(), flags.secret.clone(), 
                     flags.id.clone()).await?;
                     println!("{pwd}"); }
            _   => { }
        }
